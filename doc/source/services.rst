...
.. class:: cloudbaseinit.metadata.services.gportal.GPortalService

GPortal metadata is offered via an internal HTTP metadata endpoint, reachable at the magic URL
`http://169.254.169.254/metadata/`.
You can access the full response as JSON by using the URL `http://169.254.169.254/metadata/v1.json`.

The Metadata API documentation is not publicly available at the moment and will be shared in future.
Below you can find a example response including all metadata information that are currently supported by GPortal.
...
Config options for `gportal` section:
